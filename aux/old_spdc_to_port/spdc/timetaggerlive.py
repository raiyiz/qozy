...
class timetagger_device:
    ...
    def setup_sm(self):
        self.sm = TimeTagger.SynchronizedMeasurements(
            self.tagger
        )  # Allows for synchronizing multiple measurement classes in a way that ensures all these measurements to start, stop simultaneously and operate on exactly the same time tags
    ...
    def setup_counters(self, channel_list, CountsBinwidth, CountsTimeFrame):
        ...
        self.counter = TimeTagger.Counter(
            self.sm_tagger, channel_list, CountsBinwidth * 1e9, CountsBinNumber
        )
    ...
    def setup_coincidences(self, a_channels, b_channels, CoinTimeWindow):
        ...
        self.coin = TimeTagger.Coincidences(
            self.sm_tagger, coin_channel_combinations, CoinTimeWindow * 1e3
        )
        ...
        for b in b_channels:
            corr_list.append(
                TimeTagger.Correlation(
                    self.sm_tagger, a_channels[0], b, CorrBinWidth * 1e3, CorrBinNumber
                )
            )
