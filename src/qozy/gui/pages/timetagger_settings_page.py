...
class TimeTaggerSettingsPage(QWidget):
    ...
    def export_config(self, config: AppConfig) -> None:
        """Persist connection selection and the current Time Tagger profile."""
    ...
    def set_busy(self, busy: bool) -> None:
        enabled = not busy and self._thread is None
        for widget in (
            self.backend_combo,
    ...
    def _build_connection_card(self) -> QWidget:
        ...
        for label, _backend in self._BACKENDS:
    ...
    def _restore_backend_selection(self) -> None:
        backend = self._initial.acquisition_backend
        names = [name for _label, name in self._BACKENDS]
        if backend in names:
            self.backend_combo.setCurrentIndex(names.index(backend))
    ...
    def _backend_changed(self, _index: int) -> None:
        self.network_address.setEnabled(
            ...
            and not self.hardware.connected
            and self._selected_backend() == "timetagger-network"
        )
        self._update_connection_controls()
    ...
    def _update_device_status(self) -> None:
        ...
        try:
            self.device_label.setText(info() if info is not None else "Connected")
    ...
    def _update_connection_controls(self) -> None:
        ...
        self.network_address.setEnabled(
            not connected and self._selected_backend() == "timetagger-network"
        )
    ...
    def _start_hardware_worker(
        ...
        self.status_label.setText(
            {"connect": "Connecting…", "disconnect": "Disconnecting…", "configure_timetagger": "Applying settings…", "read_timetagger_settings": "Reading device…"}.get(action, "Working…")
        )
        self._thread.start()
