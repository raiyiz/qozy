"""Application settings and acquisition-backend connection controls.

Polarization-stage configuration lives in ``PolarizationPage``; this page
only owns the acquisition backend (Simulator / Time Tagger local / Time
Tagger network) and the export directory.
"""
...
class SettingsPage(QWidget):
    ...
    def __init__(self, hardware: HardwareManager, initial: AppConfig | None = None) -> None:
        ...
        backend_names = [name for _label, name in self._BACKENDS]
        if self._initial.acquisition_backend in backend_names:
            self.backend.setCurrentIndex(backend_names.index(self._initial.acquisition_backend))
        ...
        self.export_dir = QLineEdit(self._initial.export_dir)
